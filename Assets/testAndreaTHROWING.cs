...
public class testAndreaTHROWING : MonoBehaviour
{
    ...
    private void DotsSpawner()
    {
        ...
        else if (throwStance && Input.GetKey((control.KeyUp)) && speedX <= 45f)
        {       // adjust trajectory with 10 x limits 
            foreach (GameObject Dots in trajectoryDots)
            {
                Destroy(Dots);
            }
        ...
        else if (throwStance && Input.GetKey((control.KeyDown)) && speedX >= 5f)
        {       // adjust trajectory with 5 x limits 
            foreach (GameObject Dots in trajectoryDots)
            {
                Destroy(Dots);
            }
